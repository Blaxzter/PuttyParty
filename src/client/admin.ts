...
function dpRender(cal: HTMLElement, year: number, month: number): void {
    ...
    for (let i = 0; i < 42; i++) {
        ...
        const isSel =
            !!sel && sel.y === yy && sel.m === mm + 1 && sel.d === dd;
...
function dpOpenFor(dp: HTMLElement): void {
    ...
    dpRender(
        cal,
        sel ? sel.y : now.getFullYear(),
        sel ? sel.m - 1 : now.getMonth(),
    );
...
function dpShiftMonth(cal: HTMLElement, delta: number): void {
    ...
    cal.querySelector<HTMLElement>(
        delta < 0 ? "[data-dp-prev]" : "[data-dp-next]",
    )?.focus();
}
...
function dpFocusDate(cal: HTMLElement, date: Date): void {
    ...
    if (Number(cal.dataset.y) !== y || Number(cal.dataset.m) !== m)
        dpRender(cal, y, m);
