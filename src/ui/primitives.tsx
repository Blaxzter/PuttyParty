...
export const GolfBall: FC<{ size?: number; style?: string }> = ({ size = 14, style }) => (
  <span
    class="pp-ball"
...
export const FlagMark: FC<{ size?: number; pole?: string; flag?: string; animate?: boolean }> = ({
  size = 16,
  pole = '#16261F',
  flag = '#E2533B',
  animate = true,
...
export const BrandBadge: FC<{ size?: number; withBall?: boolean; bg?: string }> = ({
  size = 46,
  withBall = false,
  ...
  >
    ...
    {withBall ? (
      <GolfBall
        size={Math.round(size * 0.29)}
        style={`position:absolute;left:${size * 0.33}px;bottom:${size * 0.17}px`}
      />
    ) : null}
  </span>
)
...
export const QrImg: FC<{
  ...
  >
    <img src={`/g/${publicId}/qr?target=${target}`} alt="QR-Code" width={size} height={size} />
  </span>
)

// ---- Toast (SSR fallback; the client also builds these dynamically) ----

export const Toast: FC<{ message: string; id?: string }> = ({ message, id }) => (
...
const CONFETTI = [
  { left: 16, top: 18, w: 8, h: 12, color: '#F2C14E', round: 2, dur: 2.4, delay: 0 },
  { left: 38, top: 12, w: 9, h: 9, color: '#E2533B', round: 50, dur: 2.8, delay: 0.4 },
  { left: 62, top: 16, w: 8, h: 12, color: '#2E8B57', round: 2, dur: 2.2, delay: 0.7 },
  { left: 80, top: 13, w: 9, h: 9, color: '#D8A72B', round: 50, dur: 2.6, delay: 0.2 },
  { left: 50, top: 20, w: 8, h: 12, color: '#F2C14E', round: 2, dur: 3, delay: 1 },
]
...
export const BrowserlessHead: FC<{ eyebrow?: string; title: string; children?: Child }> = ({
  eyebrow,
  title,
  children,
}) => (
  <div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;flex-wrap:wrap">
    <div>
      {eyebrow ? (
        <div class="pp-mono" style="font-size:11px;color:var(--pp-text-faint);margin-bottom:2px">
          {eyebrow}
        </div>
      ) : null}
      <h2 class="pp-h" style="margin:0;font-weight:800;font-size:26px;color:var(--pp-turf-to)">
        {title}
      </h2>
    </div>
    {children}
