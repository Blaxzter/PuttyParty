...
function strokeField(msg = 'Bitte eine Zahl ab 1 eingeben.') {
  return z.preprocess(
    ...
    z.number({ error: msg }).int(msg).min(1, msg).max(999, 'Maximal 999 Schläge.'),
  )
}
...
export function perHoleEntrySchema(holes: number) {
  return z.object({
    name: nameField,
    team: optionalText(60),
    holeStrokes: z.array(holeStrokeField()).length(holes, 'Bitte alle Bahnen ausfüllen.'),
  })
}
...
export const createGameSchema = z.object({
  ...
  holes: z.preprocess(
    ...
    z.number().int().min(1, 'Mindestens 1 Bahn.').max(60, 'Maximal 60 Bahnen.'),
  ),
